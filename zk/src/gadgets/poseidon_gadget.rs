...
fn Poseidon_permutation(input: &[Scalar], params: &PoseidonParams, sbox: &SboxType) -> Vec<Scalar> {
...
pub fn Poseidon_hash_8_gadget<'a, CS: ConstraintSystem>(
    ...
    let mut input_arr: [LinearCombination; 8] = [
        LinearCombination::default(),
        LinearCombination::default(),
        LinearCombination::default(),
        LinearCombination::default(),
...
mod tests {
    ...
    fn poseidon_perm(width: usize, sbox_type: &SboxType, transcript_label: &'static [u8]) {
        ...
        let (proof, commitments) = {
            ...
            debug!(
                "For Poseidon permutation rounds {}, metrics: {:?}",
                total_rounds,
                &prover.metrics()
            );
    ...
    fn poseidon_hash_2(sbox_type: &SboxType, transcript_label: &'static [u8]) {
        ...
        let (proof, commitments) = {
            ...
            debug!(
                "For Poseidon hash 2:1 rounds {}, metrics: {:?}",
                total_rounds,
                &prover.metrics()
            );
    ...
    fn poseidon_hash_4(sbox_type: &SboxType, transcript_label: &'static [u8]) {
        ...
        let (proof, commitments) = {
            ...
            let start = Instant::now();
            assert!(
                Poseidon_hash_4_gadget(&mut prover, allocs, &s_params, sbox_type, &expected_output)
                    .is_ok()
            );

            debug!(
                "For Poseidon hash 4:1 rounds {}, metrics {:?}",
                total_rounds,
                &prover.metrics()
            );
    ...
    fn poseidon_hash_8(sbox_type: &SboxType, transcript_label: &'static [u8]) {
        ...
        let (proof, commitments) = {
            ...
            let start = Instant::now();
            assert!(
                Poseidon_hash_8_gadget(&mut prover, allocs, &s_params, sbox_type, &expected_output)
                    .is_ok()
            );

            debug!(
                "For Poseidon hash 8:1 rounds {}, metrics {:?}",
                total_rounds,
                &prover.metrics()
            );
