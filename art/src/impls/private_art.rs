// Asynchronous Ratchet Tree implementation for PrivateART
    ...
    G::BaseField: PrimeField,
{
    pub fn public_key_of(&self, secret: &G::ScalarField) -> G {
        self.public_art.get_generator().mul(secret).into_affine()
    }
    ...
    G::BaseField: PrimeField,
{
    ...
    pub fn update_key(
        ...
        let (tk, changes, artefacts) = self.public_art.update_art_branch_with_leaf_secret_key(
            new_secret_key,
    ...
    pub fn make_blank(
        ...
        let append_changes = matches!(
            self.public_art.get_node_with_path(&path)?.get_status(),
    ...
    pub fn update_private_art(&mut self, changes: &BranchChanges<G>) -> Result<(), ARTError> {
        if let BranchChangesType::MakeBlank = changes.change_type
            && matches!(
                self.public_art.get_node(&changes.node_index)?.get_status(),
                Some(LeafStatus::Blank)
            )
        {
    ...
    G::BaseField: PrimeField,
{
    ...
    pub(crate) fn update_node_index(&mut self) -> Result<(), ARTError> {
        let path = self
            .public_art
            .get_path_to_leaf(&self.public_art.public_key_of(&self.get_secret_key()))?;
    ...
    fn update_private_art_with_options(
        ...
        self.public_art
            .update_public_art_with_options(changes, append_changes, update_weights)?;
