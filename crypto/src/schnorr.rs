...
use zkp::{
    self, CompactProof, Transcript,
    ark_ec::AffineRepr,
    toolbox::{FromBytes, SchnorrCS, ToBytes, prover::Prover},
};

use crate::CryptoError;

/// sign a message using multiple secret keys with Schnorr signature scheme
pub fn sign<G: AffineRepr>(
    sk: &Vec<G::ScalarField>,
    Q: &Vec<G>,
    msg: &[u8],
) -> Result<Vec<u8>, CryptoError> {
    if sk.len() != Q.len() {
        return Err(CryptoError::SchnorrError(
            "Secret keys and public keys must have the same length".to_string(),
        ));
    }
